...
from VQSynergy.config import DEVICE
...
def calculate_graph_feat(feat_mat, adj_list):
    ...
    assert np.allclose(adj_mat, adj_mat.T)  # it's symmetric

    # compact the adjacency matrix to another expression
    # shape: |E| x 2
...
def drug_feature_extract(drug_data):
    # number of rows == length of atom features
    drug_data = pd.DataFrame(drug_data).T
    drug_feat = [[] for _ in range(len(drug_data))]
    for i in range(len(drug_feat)):
        # feat_mat: atom's features

        # featurizer = dc.feat.ConvMolFeaturizer()
        # x = C1=CC(=CC=C1C[C@@H](C(=O)O)N)N(CCCl)CCCl
        # feat_mat = featurizer.featurize(Chem.MolFromSmiles(x)).get_atom_features()
